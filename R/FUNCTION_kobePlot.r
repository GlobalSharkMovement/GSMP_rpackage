...
kobePlot <- function(dta, xNam = 'mean_effort', yNam = 'mean_overlap', gp = 'Spp',
                     ...
                     cutY = 100, yBks = seq(0,cutY, length.out = 5),...)
  {
    ...
    kobe <- kobe +
      ggplot2::geom_rect(xmin = 0, xmax = F40.F20, ymin = 0, ymax = So,
                fill = alpha('green3',0.2), col = 'black', linetype = 'dotted')
    # Yellow sqare (Bottom right)
    kobe <- kobe +
      ggplot2::geom_rect(xmin = F40.F20, xmax = xxmax, ymin = 0, ymax = So,
                fill = alpha('gold',0.2), col = 'black', linetype = 'dotted')#, alpha = 0.015)

    # Yellow sqare (top Left)
    kobe <- kobe +
      ggplot2::geom_rect(xmin = 0, xmax = F40.F20, ymin = So, ymax = 100,
                fill = alpha('gold',0.2), col = 'black', linetype = 'dotted')#, alpha = 0.015)

    # Red sqare (top Right)
    kobe <- kobe +
      ggplot2::geom_rect(xmin = F40.F20, xmax = xxmax, ymin = So, ymax = 100,
                fill = alpha('red2',0.2), col = 'black', linetype = 'dotted')#, alpha = 0.015)

    # SD bars
    kobePts <- kobe +
      # Vertical bars (y axis)
      ggplot2::geom_errorbar(aes(x = eval(parse(text = xNam)),
      ...
      ggplot2::geom_errorbarh(aes(y = eval(parse(text = yNam)),
    ...
    if(gline)
    {
      ...
      kobePts <- kobePts + ggplot2::geom_path(aes(x = eval(parse(text = xNam)),
                                          y = eval(parse(text = yNam))))
    }
    ...
    # Points
    kobePts <- kobePts + ggplot2::geom_point(aes(x = eval(parse(text = xNam)),
                                        y = eval(parse(text = yNam)),
    ...
    if(!is.null(colPlt))
    {
      kobePts <- kobePts + ggplot2::scale_colour_manual(values = colPlt)
    }
